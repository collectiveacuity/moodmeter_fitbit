    ...
    let callback_map = {
      ...
      report_callback: function(content){
        ...
        // show tip
          ...
          let log_msg = "Mood Reminder: " + content.msg
  ...
  moods(mood_settings=null) {
    ...
    // Bind listener to button
      moodButton.onclick = function(evt){
        ...
        // retrieve date and current heart rate reading and add to mood report
        if (self.hrm.bpm == null){
          self.hrmStatus()
          self.hrm.reading().then(function(bpm){
            _sending_data(bpm)
          }).catch(function(err){
            _sending_data(null)
          })
        } else {
          _sending_data(self.hrm.bpm)
        }
